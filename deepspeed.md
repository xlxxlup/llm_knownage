...
模型训练参数的完整流程：完整训练流程（一步一步对应你的疑问）

每一步训练，严格按这个顺序走：

### 1. 拷贝：FP32 主参数 → 复制成 FP16 计算参数

- 把高精度的**FP32 原版参数**，拷贝一份低精度的**FP16 副本**
- 这是你看到 FP16 参数的来源！

### 2. 计算（前向 + 反向）：全程用 FP16

- **前向传播**：用 FP16 参数算 loss（快、省显存）
- **反向传播**：用 FP16 参数算出**FP16 梯度**（临时的）

### 3. 梯度转换：FP16 梯度 → 转成 FP32 梯度

- FP16 太小了，极小的梯度会直接变成**0（下溢）**，训练直接崩
- 所以必须转成 FP32 梯度，安全累加

### 4. 参数更新：只用 FP32（高精度）

- 优化器（AdamW/SGD）拿着**FP32 梯度**，更新**FP32 主参数**
- 这一步是模型真正学习的环节，**绝对不用 FP16**

### 5. 循环

- 清空梯度，下一轮再次把 FP32 拷贝成 FP16，重复计算
