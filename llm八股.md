...
> **全连接层是用一套固定的“模版”去硬套所有数据，而 Attention 是用数据自己生成的“关系”去动态对齐。**

你的观察中关于 **Query 是锚点** 的观点非常深刻，这正是为什么 Attention 具有“自适应性”的根本原因。
...
为了节省显存，Flash Attention 在前向传播时**不存储** $N \times N$ 的注意力矩阵（这个矩阵是显存爆炸的元凶）。
...
| 折扣因子     | γ（Gamma）      | 用于权衡即时奖励和未来奖励的系数（γ∈[0,1]）                  | γ=0.9：重视未来奖励（愿意多走几步找最优路径）；γ=0：只关注即时奖励（短视行为） |               |                                                              |                |
...
![image-20251220134638824](./llm八股.assets/image-20251220134638824.png)

一般python写的MCP Server 使用uvx启动，node写的MCP Server 使用npx启动

![image-20251220141501950](./llm八股.assets/image-20251220141501950.png)
...
![image-20251220160712985](./llm八股.assets/image-20251220160712985.png)

**cline 进行工具调用与MCP Server的交互内容**

![image-20251220160917697](./llm八股.assets/image-20251220160917697.png)

#### **cline 与 模型的交互过程**

**cline向大模型发起请求时，prompt会携带很多信息**

![image-20251220170651324](./llm八股.assets/image-20251220170651324.png)

![image-20251220170726661](./llm八股.assets/image-20251220170726661.png)

**模型每一步都会先进行思考<thinking>，再做决策<use_mcp_tool>，cline再通过MCP Server去调用工具，cline再将工具结果返回给模型(observation)**

![image-20251220170225458](./llm八股.assets/image-20251220170225458.png)

**<attempt_completion>:模型认为任务已完成，最终返回给cline的信息。**

## A2A

![image-20251220180626594](./llm八股.assets/image-20251220180626594.png)
...
![image-20251220175913476](./llm八股.assets/image-20251220175913476.png)

agent card

![image-20251220180210107](./llm八股.assets/image-20251220180210107.png)



![image-20251220180230931](./llm八股.assets/image-20251220180230931.png)

![image-20251220180246869](./llm八股.assets/image-20251220180246869.png)

![image-20251220180448394](./llm八股.assets/image-20251220180448394.png)

agent问答阶段

![image-20251220175834021](./llm八股.assets/image-20251220175834021.png)
