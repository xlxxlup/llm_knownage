...
- **Multi-Head Attention**: 这是 Transformer 中的核心改进，通过同时使用多组独立的注意力头（多个 QKV 三元组）并行计算 Scaled Dot-Product Attention，并在输出时将各头结果拼接后线性变换；允许模型在不同的子空间上学习不同类型的注意力模式（如局部依赖、全局依赖）。
...
- **Flash Attention**: 基于显存优化的 Scaled Dot-Product Attention 变体，通过**分块（tiling）**和**重计算（recomputation）**减少显存占用，同时利用 GPU 内存层级特性提升计算速度；解决了长序列 Attention 的显存瓶颈，是当前工业界长文本模型的主流实现。
