...
渐进式训练：先在小的llm上训vit，之后直接移到大的llm上
...
为了提升模型对**真实世界场景的适应能力和整体表现**，文中引入了两项核心技术：**随机 JPEG 压缩** 和 **损失重加权**。
...
[(2 封私信 / 22 条消息) InternVL3：刷新开源多模态大模型性能新纪录 - 知乎](https://zhuanlan.zhihu.com/p/1897681159359551408)

**预训练**：

**原生多模态预训练**，**语言预训练和多模态对齐训练整合到单一预训练阶段的原生方法**。与先训练文本模型再适配模态的传统范式不同，InternVL3 通过在预训练过程中交错处理多模态数据和大规模文本语料库来实现**集成优化**

**后训练**：

**SFT**:模型通过积极监督信号模仿高质量响应。沿用了 InternVL2.5 ([18]) 中的随机 JPEG 压缩、平方损失重加权和多模态数据打包等技术。核心改进在于使用**更高质量、更多样化的训练数据**。



# JPEG 压缩（超通俗版）

**JPEG 就是给照片“减肥”的算法，属于有损压缩。

它利用人眼的视觉弱点，扔掉你几乎看不见的信息，让图片体积大幅变小。**
...
这一步就叫**色度降采样**（比如 4:2:0），颜色信息直接少掉一大半，你却看不出来。

## 2. 图片会被切成小块

JPEG 会把图片切成很多 **8×8 像素的小方块**，一块一块压缩。

## 3. 把图片变成“高频细节 + 低频轮廓”

通过一个数学变换（DCT），把每个小块拆成：
...
你设置的“画质 80 / 50 / 10”，就是控制扔多少高频信息。

## 4. 最后再无损压缩一遍

扔掉不重要信息后，再用哈夫曼编码把重复数据压一压，这一步是**无损**的。

# 整体流程一句话

**色彩瘦身 → 切成8×8小块 → 扔掉细小细节 → 再压缩打包 → 变小的JPEG图片**

# 关键特点

- **有损压缩**：信息一旦扔掉，再也找不回来
- **适合照片、风景、人像**：颜色渐变多、细节柔和
- **不适合文字、线条、图标**：会出现模糊、方块伪影
- **反复保存会越来越糊**：每次保存都再扔一次信息

# 和 PNG 对比

- **JPEG**：有损、体积小、适合照片
- **PNG**：无损、体积大、适合文字、图标、透明背景

两阶段后训练：

1、SFT:在此阶段，InternVL3系列还采用了InternVL2.5中提出的随机JPEG压缩、平方损耗重权和多模数据打包技术。InternVL3 SFT 阶段相较于 InternVL2.5  的**主要进展在于使用更高质量和更多样化的训练数据。具体来说，我们还进一步扩展了工具使用、三维场景理解、图形界面操作、长上下文任务、视频理解、科学图表、创意写作和多模态推理的训练样本**。

2、MPO: 引入了来自正负样本的额外监督，**将模型响应分布与真实分布对齐，提高推理性能**。MPO 目标函数是**偏好损失 Lp、质量损失 Lq 和生成损失 Lg 的组合** ([9])。**Lp 使用 DPO 损失学习选择响应和拒绝响应的相对偏好。Lq 使用 BCO 损失 帮助模型理解个体响应的绝对质量。Lg 使用 LM 损失学习首选响应的生成过程**。MPO 数据基于 MMPR v1.2 ([124]) 的数据管道和样本构建，涵盖多种领域（VQA、科学、图表、数学、OCR、文档）。使用 InternVL3-8B、38B、78B 的 SFT 版本生成 rollouts。MPO 阶段所有模型在相同数据集上训练，约包含 **30 万**样本。
...
![image-20260331180423493](./image/image-20260331180423493.png)
...
![image-20260331180436834](./image/image-20260331180436834.png)

![image-20260331180449571](./image/image-20260331180449571.png)
...
**在训练阶段，作者并不是固定死一个 $\delta$（比如 0.5），而是动态随机采样不同的 $\delta$。**
...
（1）原生预训练以实现视觉语言对齐(平方根平均损失)

（2）监督微调以适应下游任务

（3）级联强化学习以提升推理能力。InternVL3.5-Flash是InternVL3.5的高效版本，通过一致性训练和路由器训练进一步集成了视觉分辨率路由器（ViR）



我们在InternVL3.5中提出了三项创新设计，包括级联强化学习（Cascade RL）、**视觉分辨率路由器（ViR）动态选择视觉token的最低分辨率**，从而实现更好的推理效率，和**解耦视觉语言部署（DvD）**。这些技术显著提升了InternVL3.5的能力和效率，为社区提供了实用建议。
...
3、**视觉一致性学习（ViCO）**旨在将**视觉分辨率路由器**（ViR）集成到InternVL3.5中，通过**最小化不同视觉压缩率的输出差异**来构建InternVL3.5-Flash。
