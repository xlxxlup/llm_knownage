...
- **做法**：统一缩小所有频率分量的旋转角度，全局等比例缩放（即对 RoPE 的所有 θ_i 按同一比例缩放，使长上下文的位置编码适配原训练分布）。
- **致命问题**：依据 NTK 理论，**统一压缩会丢失高频信息**，模型无法区分近距离、语义相似的 token，细节损失极大，长文本场景下语义连贯性严重下降。

### 2. NTK-aware Interpolation（NTK 感知内插）

- **核心**：修复 PI 的高频丢失，遵循**高频少缩、低频多缩**，平衡长上下文适配与高频细节保留。

  **做法**：按频率差异化缩放 —— **高频尽量不缩（保留细节），低频多缩（适配长上下文），用指数函数关联频率与缩放程度**（对 RoPE 的 θ_i 按频率指数差异化缩放，i 越小（低频）缩放幅度越大，i 越大（高频）缩放幅度越小）。

  NTK 为了保住细节，让**高频少缩放、低频多缩放**；

  低频因为缩得多，旋转角（pos×θ）增长慢，始终处于训练见过的角度范围，不会外推；

  高频因为缩得太少，长上下文下旋转角暴增，**超出训练分布，形成高频过度外推**，**导致模型注意力紊乱、数值不稳定、性能下降**。

### 3. NTK-by-parts Interpolation（分段 NTK 内插）

**核心**：精细化频段控制，解决 NTK-aware 的**高频过度外推**问题。

**做法**：按波长分三段处理，精准控制各频段缩放策略，避免高频过度外推的同时，兼顾低频稳定性与高频细节：

- 极低频：**完全内插**（按比例大幅缩放 θ_i，确保旋转角不超出训练分布，绝不外推，保障基础上下文适配稳定性）

- 高频：**完全外推**（不缩放 θ_i，全力保留高频细节，同时通过分段限制，避免旋转角暴增超出训练范围）

  **高频部分几乎没有被压缩**（保持原样），这在长上下文场景下导致高频维度仍然需要处理超出训练分布的位置

- 中间频：内外插混合过渡（平滑衔接极低频与高频的缩放策略，避免频段切换导致的性能波动）
...
- 做法
  ...
  **温度缩放的目的**：你提到"稳定长文本注意力分布"，这没错。但更精确地说，是为了解决**Softmax Sharpening**问题——**位置压缩导致 Q 和 K 的夹角变小，点积分数被人为放大，使得注意力分布过于尖锐（over-sharpening），温度缩放通过"软化"softmax 来缓解这一问题** 
...
![image-20260414222835828](./image/image-20260414222835828.png)
