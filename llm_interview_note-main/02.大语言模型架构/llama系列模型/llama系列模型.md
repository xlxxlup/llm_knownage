...
LLaMA 所采用的 Transformer 结构和细节，与标准的 Transformer 架构不同的地方包括采用了**前置层归一化（Pre-normalization）**并使用 **RMSNorm 归一化函数** （Normalizing Function）、**激活函数更换为** SwiGLU****，并使用了**旋转位置嵌入（RoPE）**，整体 Transformer 架构与 GPT-2 类似。
