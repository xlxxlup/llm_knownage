...
![image-20260409173934050](./image/image-20260409173934050.png)

**1、先使用old模型对于一个问题采样得到多个回答**

为什么不采样一次更新一次：即你说的「采样一次更新一次」的最原始实现）具有高方差，这会导致**学习缓慢和训练不稳定，同时需要极大的算力**，浪费了算力资源
...
- **为什么第三个读：** 这是**“工程与实战”的集大成者**。
  ...
  - #### 1. 解耦 Clip（Decoupled Clip）
  
    - **问题**：**GRPO 对称 Clip 限制了低概率 token 的探索**，**容易让模型过早陷入单一推理路径（熵坍缩）**。
    - **改进**：把 Clip 上下限拆开，**给「提升低概率 token」更大的空间**（比如允许向上多涨一点，向下少限一点）。
    - **好处**：鼓励模型探索更多可能正确的路径，提升复杂推理任务的表现。
  
    #### 2. 动态采样（Dynamic Sampling）
  
    - **问题**：**批量采样里如果一组样本全对 / 全错，优势值为 0，梯度没用**，纯浪费 GPU 算力。
    - **改进**：实时过滤掉「奖励无差异」的样本，重新采样直到这组样本能提供有效梯度信号。
    - **好处**：数据效率大幅提升，训练更快，减少无效计算。
  
    #### 3. Token 级损失（Token-Level Loss）
  
    - **问题**：**GRPO 序列级平均损失会稀释关键推理步骤的梯度，长文本（比如多步数学证明）训练不稳定**。
    - **改进**：**直接对每个 token 的损失求和，不做序列内平均**。
    - **好处**：**保留关键步骤的梯度权重，长文本训练更稳，收敛更快**。
  
    #### 4. 软过长惩罚（Soft Overlong Punishment）
  
    - **问题**：**生成文本超过长度上限被截断时，奖励和梯度会失真，干扰训练**。
    
    - **改进**：**对接近上限的样本加平滑惩罚，极端过长的样本直接丢弃**。
    
    - **好处**：缓解奖励噪声，让长文本生成的训练更稳定。
    
      ![image-20260413101842547](./image/image-20260413101842547.png)
  
  ### **移除了KL：**
  
  KL惩罚项用于调节在线策略和冻结参考策略之间的差异。在 RLHF 场景 [23]  中，强化学习的目标是调整模型行为，但不会偏离初始模型太远。然而，**在训练长CoT推理模型期间，模型分布可能与初始模型显着偏离，因此这种限制是没有必要的。因此，我们将从我们提出的算法中排除 KL 项**。

#### 4. ASPO (Asymmetric Importance Sampling Policy Optimization)

- **作者：** Wang 等
- ASPO：（1）防止过拟合和熵坍缩，（2）实现更平滑稳定的训练动态，（3）在最终性能上显著优于基于GRPO的基线
- **为什么第四个读：** 这是**“数学细节”的精修**。
  
  - 它深入到了 PPO/GRPO 的核心公式——**重要性采样（Importance Sampling, IS）**。
  - 它发现一个被大家忽略的问题：
  
    原来的公式对 **“好样本”（该提高概率）**和**“坏样本”（该降低概率）**的**更新力度是不对称、不公平的 **—— 要么压得太狠，要么放得太开，容易让训练震荡。
