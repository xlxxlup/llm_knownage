...
| 方法                         | 公式                                 | 特点                     |
| ---------------------------- | ------------------------------------ | ------------------------ |
| **欧几里得距离 (Euclidean)** | $\sqrt{\sum_{i=1}^{n}(x_i - y_i)^2}$ | 直观几何距离，对维度敏感 |
| **曼哈顿距离 (Manhattan)**   | $\sum_{i=1}^{n}\|x_i - y_i\|$        | 适合高维稀疏数据         |

| 方法                    | 公式                                                         | 适用场景                       |
| ----------------------- | ------------------------------------------------------------ | ------------------------------ |
| **余弦相似度 (Cosine)** | $\frac{\mathbf{x} \cdot \mathbf{y}}{\|\mathbf{x}\| \|\mathbf{y}\|}$ | 最常用，忽略向量长度，关注方向 |

- **KL 散度 / JS 散度**：衡量概率分布差异
