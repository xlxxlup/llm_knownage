...
**2. 命题分块 (Proposition Chunking)**
- **核心思想**: 基于语义命题(独立事实单元)进行分块
- **优势**: 保持语义完整性,避免重要信息被拆分
- **实现**: 使用NLP技术识别文本中的独立命题
- **场景**: 法律文档、技术手册等需要精确语义的场景

**3. 上下文块标题 (Contextual Chunk Headers, CCH)**
- **核心思想**: 为每个文本块添加高层级上下文标题
- **实现**: 在嵌入前添加"文档标题-章节标题-子章节"前缀
- **示例**:
  ```
  原始块: "样本量为50,有效率达80%"
  增强后: "《AI医疗白皮书》-第三章 诊断技术 - 3.2 影像识别实验: 样本量为50,有效率达80%"
  ```
- **效果**: 检索准确率提升15-30%,减少指代歧义
- **成本**: 低(仅需预处理阶段添加)

**4. 文档增强 (Document Augmentation)**
- **核心思想**: 为每个文本块生成相关问题
- **流程**:
  1. 使用LLM为文档生成3-5个相关问题
  2. 将问题与原文合并嵌入
  3. 检索时匹配用户查询与生成问题
- **优势**: 桥接"文档表述"与"用户查询"的语义鸿沟
- **场景**: 客服知识库、教育材料

#### 1.2 多模态支持

**5. 多模态RAG (Multi-modal RAG)**
- **方案A - 基于Captioning**:
  - 使用视觉模型生成图像描述
  - 将文本+描述联合索引
...
**13. 多面过滤 (Multi-faceted Filtering)**
- **核心思想**: 结合向量检索与元数据过滤
- **示例**:
  ```
  查询: "2024年AI市场规模"
...
**14. Dartboard检索 (Dartboard RAG)**
- **核心问题**: 解决Top-k检索的内容冗余问题
- **评分公式**:
  ```
  综合分数 = α × 相关性分数 + β × 多样性分数
  ```
- **多样性计算**: 文档与已选文档的语义距离
- **效果**: 信息覆盖度提升30-50%
- **论文**: [Better RAG using Relevant Information Gain](https://arxiv.org/abs/2407.12101)

**15. 可解释检索 (Explainable Retrieval)**
- **核心思想**: 为每个检索结果生成相关性解释
- **实现**:
  ```
  检索文档 + LLM解释生成
  → 输出: "文档包含查询关键词'Python性能优化',并提供了'列表推导式'方案"
  ```
- **价值**:
  - 增强用户信任
  - 便于调试优化
